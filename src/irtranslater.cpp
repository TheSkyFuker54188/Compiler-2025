...
void RiscvBlock::InsertInstruction(int pos, std::unique_ptr<RiscvInstruction> ins) {
  if (pos == 0) {
      ...
      if (alloca_inst) {
        ...
        if (reg_operand) {
          if (!alloca_inst->GetDims().empty()) {
            // 数组分配
            addLocalArray(reg_operand->GetRegNo(), 
                         alloca_inst->GetType(), alloca_inst->GetDims());
          } else {
            // 标量分配
            addLocalVariable(reg_operand->GetRegNo(), 
                            alloca_inst->GetType());
          }
        }
      }
...
void Translator::translateInstruction(Instruction inst, RiscvBlock *riscv_block) {
  switch (inst->GetOpcode()) {
    ...
    case LLVMIROpcode::FDIV:
      translateArithmetic(dynamic_cast<ArithmeticInstruction*>(inst), riscv_block);
      break;
    ...
    default:
      // 对于不支持的指令，添加注释
      // riscv_block->InsertInstruction(1, std::make_unique<RiscvPseudoInstruction>(RiscvOpcode::NOP, nullptr));
      break;
  }
}

void Translator::translateArithmetic(ArithmeticInstruction *inst, RiscvBlock *block) {
  if (!inst) return;
  
  RiscvOpcode riscv_op = llvmToRiscvOpcode(static_cast<LLVMIROpcode>(inst->GetOpcode()), 
...
void Translator::translateLoad(LoadInstruction *inst, RiscvBlock *block) {
  ...
  RiscvOpcode op = (inst->GetType() == LLVMType::FLOAT32) ? 
    RiscvOpcode::FLW : RiscvOpcode::LW;
  
  auto rd = translateOperand(inst->GetResult());
  auto addr = translateOperand(inst->GetPointer());
  
  auto riscv_inst = std::make_unique<RiscvMemoryInstruction>(
    op, std::move(rd), std::move(addr));
...
void Translator::translateStore(StoreInstruction *inst, RiscvBlock *block) {
  ...
  RiscvOpcode op = (inst->GetType() == LLVMType::FLOAT32) ? 
    RiscvOpcode::FSW : RiscvOpcode::SW;
  
  auto rs = translateOperand(inst->GetValue());
  auto addr = translateOperand(inst->GetPointer());
  
  auto riscv_inst = std::make_unique<RiscvMemoryInstruction>(
    op, std::move(rs), std::move(addr));
...
void Translator::translateBranch(Instruction inst, RiscvBlock *block) {
  if (inst->GetOpcode() == LLVMIROpcode::BR_UNCOND) {
    ...
    if (br_inst) {
      ...
      auto true_label_op = dynamic_cast<LabelOperand*>(br_inst->GetTrueLabel());
      ...
      auto riscv_inst = std::make_unique<RiscvBranchInstruction>(
        RiscvOpcode::BNE, std::move(cond), std::move(zero), std::move(true_label));
      block->InsertInstruction(1, std::move(riscv_inst));
      
      // 为false分支添加无条件跳转
      auto false_label_op = dynamic_cast<LabelOperand*>(br_inst->GetFalseLabel());
...
void Translator::translateReturn(RetInstruction *inst, RiscvBlock *block) {
  ...
  auto ret_inst = std::make_unique<RiscvPseudoInstruction>(
    RiscvOpcode::RET, nullptr);
...
void Translator::translateIcmp(IcmpInstruction *inst, RiscvBlock *block) {
  ...
  RiscvOpcode op;
  switch (inst->GetCond()) {
    case IcmpCond::eq: op = RiscvOpcode::BEQ; break;
    case IcmpCond::ne: op = RiscvOpcode::BNE; break;
    case IcmpCond::slt: op = RiscvOpcode::SLT; break;
    case IcmpCond::sgt: // 通过交换操作数实现
    case IcmpCond::sge:
    case IcmpCond::sle:
    default: op = RiscvOpcode::SLT; break;
  }
...
void Translator::translateFcmp(FcmpInstruction *inst, RiscvBlock *block) {
  ...
  RiscvOpcode op;
  switch (inst->GetCond()) {
    case FcmpCond::OEQ: op = RiscvOpcode::FEQ_S; break;
    case FcmpCond::OLT: op = RiscvOpcode::FLT_S; break;
    case FcmpCond::OLE: op = RiscvOpcode::FLE_S; break;
    default: op = RiscvOpcode::FEQ_S; break;
  }
...
// 物理寄存器创建辅助方法
std::unique_ptr<RiscvRegOperand> Translator::createPhysicalReg(int physical_reg_no) {
  return std::make_unique<RiscvRegOperand>(-physical_reg_no); // 负数表示物理寄存器
}

std::unique_ptr<RiscvRegOperand> Translator::createVirtualReg(int virtual_reg_no) {
  return std::make_unique<RiscvRegOperand>(virtual_reg_no); // 正数表示虚拟寄存器
}
...
std::string Translator::getLLVMTypeString(LLVMType type) {
  switch (type) {
    case LLVMType::I32: return "i32";
    case LLVMType::FLOAT32: return "float";
    case LLVMType::PTR: return "ptr";
    case LLVMType::VOID_TYPE: return "void";
...
void Translator::addLocalArray(int virtual_reg, LLVMType type, const std::vector<int> &dims) {
  if (!current_stack_frame) return;
...
void Translator::updateCallArgsSize(int num_args) {
  ...
  if (num_args > 8) {
    ...
    current_stack_frame->max_call_args = std::max(current_stack_frame->max_call_args, num_args);
    current_stack_frame->call_args_size = std::max(current_stack_frame->call_args_size, args_size);
  }
}
...
void Translator::generateStackFrameProlog(RiscvBlock *entry_block) {
  if (!current_stack_frame || current_stack_frame->total_frame_size == 0) return;
  
  // 生成栈帧分配代码
  // addi sp, sp, -frame_size
  auto sp = getStackPointerReg();
  
  auto alloc_inst = std::make_unique<RiscvImmediateInstruction>(
    RiscvOpcode::ADDI, sp->CopyOperand(), sp->CopyOperand(), -current_stack_frame->total_frame_size);
  ...
  if (current_stack_frame->max_call_args > 0) {
    ...
    auto ra_offset = std::make_unique<RiscvImmI32Operand>(current_stack_frame->total_frame_size - 4);
    auto sp_copy = getStackPointerReg();
    auto addr = std::make_unique<RiscvDramOperand>(current_stack_frame->total_frame_size - 4, std::move(sp_copy));
    
    auto save_ra = std::make_unique<RiscvMemoryInstruction>(
      RiscvOpcode::SW, std::move(ra), std::move(addr));
    
    entry_block->InsertInstruction(0, std::move(save_ra));
  }
}
...
int Translator::getLocalVariableOffset(int virtual_reg) {
  if (!current_stack_frame) return 0;
...
const StackFrameInfo* Translator::getFunctionStackFrame(const std::string &func_name) const {
