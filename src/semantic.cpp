...
void SemanticAnalyzer::reportError(SemanticErrorType type, const std::string& message, 
        ...
        if (array_type) {
          ...
          if (index_count > array_dims) {
            ...
            std::vector<int> remaining_dims(
              array_type->dimensions.begin() + index_count,
  ...
  if (auto* binary = dynamic_cast<BinaryExp*>(expression)) {
    ...
    if (auto* left_basic = dynamic_cast<BasicType*>(left_type.get())) {
      if (auto* right_basic = dynamic_cast<BasicType*>(right_type.get())) {
        if (left_basic->type == BaseType::FLOAT || right_basic->type == BaseType::FLOAT) {
          return makeBasicType(BaseType::FLOAT);
        } else {
          return makeBasicType(BaseType::INT);
        }
...
SemanticAnalyzer::evaluateConstExpression(Exp* expression) {
  ...
  if (auto* lval = dynamic_cast<LVal*>(expression)) {
    auto* symbol = symbol_table.lookup(lval->name);
    if (symbol && symbol->kind == SymbolKind::CONSTANT && symbol->constant_value) {
  ...
  if (auto* binary = dynamic_cast<BinaryExp*>(expression)) {
    ...
    if (!left_val || !right_val) return std::nullopt;
    ...
    // 处理混合类型运算 (int和float)
    if ((std::holds_alternative<int>(left_val.value()) && std::holds_alternative<float>(right_val.value())) ||
        (std::holds_alternative<float>(left_val.value()) && std::holds_alternative<int>(right_val.value()))) {
  ...
  if (auto* unary = dynamic_cast<UnaryExp*>(expression)) {
    ...
    if (!operand_val) return std::nullopt;
    ...
    if (std::holds_alternative<int>(operand_val.value())) {
      ...
      switch (unary->op) {
        ...
        case UnaryOp::NOT: return value == 0 ? 1 : 0;
        default: return std::nullopt;
      }
    }
    ...
    if (std::holds_alternative<float>(operand_val.value())) {
      ...
      switch (unary->op) {
        case UnaryOp::PLUS: return value;
        case UnaryOp::MINUS: return -value;
        case UnaryOp::NOT: return value == 0.0f ? 1 : 0;
        default: return std::nullopt;
      }
    }
  }
...
SemanticAnalyzer::evaluateConstInitVal(ConstInitVal* init_val) {
  if (!init_val) return std::nullopt;
...
void SemanticAnalyzer::visit(ConstDef &node) {
  ...
  // 创建符号信息
  auto symbol = std::make_shared<SymbolInfo>(SymbolKind::CONSTANT, type, node.name, true);
  ...
  if (node.initializer) {
    ...
    if (dimensions.empty()) {
      ...
      if (const_value.has_value()) {
        if (current_decl_type == BaseType::INT && std::holds_alternative<int>(const_value.value())) {
          symbol->setConstantValue(std::get<int>(const_value.value()));
        } else if (current_decl_type == BaseType::FLOAT && std::holds_alternative<float>(const_value.value())) {
          symbol->setConstantValue(std::get<float>(const_value.value()));
        } else if (current_decl_type == BaseType::FLOAT && std::holds_alternative<int>(const_value.value())) {
          // int可以隐式转换为float
          symbol->setConstantValue(static_cast<float>(std::get<int>(const_value.value())));
        }
      }
    }
  }
...
void SemanticAnalyzer::visit(VarDef &node) {
  ...
  auto symbol = std::make_shared<SymbolInfo>(SymbolKind::VARIABLE, type, node.name, is_initialized);
  symbol->setArrayDimensions(dimensions);
...
void SemanticAnalyzer::visit(FuncDef &node) {
  ...
  for (const auto& param : node.parameters) {
    ...
    if (param->is_array_pointer) {
      // 数组指针参数
      std::vector<int> dimensions = calculateArrayDimensions(param->array_dimensions);
  ...
  // 创建函数符号
  auto symbol = std::make_shared<SymbolInfo>(SymbolKind::FUNCTION, func_type, node.name, true);
  
  // 插入符号表
  if (!symbol_table.insert(node.name, symbol)) {
    reportError(SemanticErrorType::REDEFINED_SYMBOL,
                "函数 '" + node.name + "' 插入符号表失败",
                node.location);
    return;
  }
...
void SemanticAnalyzer::visit(FuncFParam &node) {
  ...
  // 创建参数符号
  auto symbol = std::make_shared<SymbolInfo>(SymbolKind::PARAMETER, type, node.name, true);
  symbol->is_array_pointer = node.is_array_pointer;
  
  // 插入符号表
  if (!symbol_table.insert(node.name, symbol)) {
    reportError(SemanticErrorType::REDEFINED_SYMBOL,
                "参数 '" + node.name + "' 插入符号表失败",
                node.location);
  }
}
...
void SemanticAnalyzer::visit(ReturnStmt &node) {
  ...
  if (node.expression) {
    ...
    if (current_function_return_type) {
      ...
      if (return_expr_type) {
        if (!isTypeCompatible(*current_function_return_type, *return_expr_type)) {
          reportError(SemanticErrorType::TYPE_MISMATCH,
                      "返回类型不匹配：期望 '" + current_function_return_type->toString() + 
    ...
    // 检查void函数
    if (current_function_return_type) {
      auto* basic_type = dynamic_cast<BasicType*>(current_function_return_type.get());
...
void SemanticAnalyzer::visit(InitVal &node) {
  std::visit([this](auto& value) {
    if constexpr (std::is_same_v<std::decay_t<decltype(value)>, std::unique_ptr<Exp>>) {
