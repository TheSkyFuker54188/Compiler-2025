...
void ASTPrinter::setShowTypes(bool show) {
    show_types = show;
}

void ASTPrinter::setShowLocations(bool show) {
    show_locations = show;
}
...
void ASTPrinter::visit(InitVal &node) {
    ...
    std::visit([this](auto& val) {
        if constexpr (std::is_same_v<std::decay_t<decltype(val)>, std::unique_ptr<Exp>>) {
            print_indent();
            out << "expression: ";
            val->accept(*this);
        } else {
