        ...
        for (auto &inst : block->Instruction_list) {
          ...
          if (opcode == BR_COND) {
            ...
            if (!operands.empty()) {
              ...
              if (cond.isConstant() && operands.size() >= 3) {
                int target = (cond.int_val != 0)
                                 ? getRegisterFromOperand(operands[1])
                                 : getRegisterFromOperand(operands[2]);
...
// 链接所需的空实现（参数类型需与头文件一致）
void SSAOptimizer::replaceWithShift(BasicInstruction *&inst,
                                    BasicOperand *const &operand,
                                    int shift_amount, LLVMIROpcode opcode) {
  // 空实现
}

void SSAOptimizer::replaceWithBitwise(BasicInstruction *&inst,
                                      BasicOperand *const &operand, int mask,
                                      LLVMIROpcode opcode) {
  // 空实现
}

bool SSAOptimizer::isPureFunctionalInstruction(BasicInstruction *const &inst) {
  // 简单实现：假定算术和比较指令是纯函数式
  if (!inst)
    return false;
  int opcode = inst->GetOpcode();
  return opcode == ADD || opcode == SUB || opcode == MUL_OP ||
         opcode == DIV_OP || opcode == MOD_OP || opcode == FADD ||
         opcode == FSUB || opcode == FMUL || opcode == FDIV || opcode == ICMP ||
         opcode == FCMP || opcode == BITAND || opcode == BITOR ||
         opcode == BITXOR || opcode == SHL;
}

std::string
SSAOptimizer::generateExpressionString(BasicInstruction *const &inst) {
  if (!inst)
    return "";
...
void SSAOptimizer::replaceInstructionWithCopy(BasicInstruction *&inst,
                                              int existing_reg) {
  // 空实现
}

std::vector<SSAOptimizer::LoopInfo>
SSAOptimizer::detectLoops(const std::map<int, BasicBlock *> &blocks) {
  // 空实现
  return {};
}

void SSAOptimizer::moveInstructionsToPreheader(
    const std::unordered_set<BasicInstruction *> &instructions,
    int preheader_block, std::map<int, BasicBlock *> &blocks) {
  // 空实现
}

bool SSAOptimizer::isLoopInvariant(
    BasicInstruction *const &inst, const LoopInfo &loop,
    const std::map<int, BasicBlock *> &blocks,
    const std::unordered_set<BasicInstruction *> &invariant_instructions) {
  // 空实现
  return false;
}

std::vector<int> SSAOptimizer::getBranchTargets(BasicInstruction *const &inst) {
  // 空实现
  return {};
}
