...
struct StackFrameInfo {
  int local_vars_size = 0;        // 局部变量占用的栈空间
  ...
  // 函数调用信息
  int max_call_args = 0;          // 最大函数调用参数数量
...
private:
  ...
  // 栈帧管理
  std::map<std::string, StackFrameInfo> function_stack_frames; // 每个函数的栈帧信息
  ...
  void addLocalArray(int virtual_reg, LLVMType type, const std::vector<int> &dims);
...
public:
  // 栈帧信息查询（公开API）
  const StackFrameInfo* getFunctionStackFrame(const std::string &func_name) const;
